...
const getCoordinates = async (searchText) => {
    ...
    const url = `https://api.mapbox.com/geocoding/v5/mapbox.places/${searchText}.json?access_token=${keys.mapboxDefault}`;
    const options = {
        method: "GET",
        headers: {
...
const getAddress = async (lng, lat) => {
    ...
    const url = `https://api.mapbox.com/geocoding/v5/mapbox.places/${lng},${lat}.json?access_token=${keys.mapboxDefault}`;
    const options = {
        method: "GET",
        headers: {
            "Content-Type": "application/json",
